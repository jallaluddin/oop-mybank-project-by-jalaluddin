...
async function service(bank) {
    do {
        ...
        if (service.option == "Cash Withdraw") {
            ...
            if (account) {
                ...
                if (ans.ques > account.balance) {
                    ...
                    myBank.transaction({ accountNumber: account.accountNumber, balance: newBalance });
                    console.log(`Dear, ${chalk.green.italic(name?.firstName + " " + name?.lastName)}! Your new Account Balance is ${chalk.bold.blueBright("$" + newBalance)}`);
                }
            }
        }
        ...
        if (service.option == "Exit") {
            console.log(chalk.yellow.bold("Thank you for using our service!"));
            break;
        }
    } while (true);
}
